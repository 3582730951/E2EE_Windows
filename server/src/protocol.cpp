...
bool ReadString(const std::vector<std::uint8_t>& data, std::size_t& offset,
                std::string& out) {
  return ReadString(ByteView{data.data(), data.size()}, offset, out);
}

bool ReadString(ByteView data, std::size_t& offset, std::string& out) {
  if (!data.data || offset + 2 > data.size) {
    return false;
  }
  const std::uint16_t len =
      static_cast<std::uint16_t>(data.data[offset] |
...
bool ReadStringView(ByteView data, std::size_t& offset,
                    std::string_view& out) {
  if (!data.data || offset + 2 > data.size) {
    return false;
  }
...
bool ReadUint32(const std::vector<std::uint8_t>& data, std::size_t& offset,
                std::uint32_t& out) {
  return ReadUint32(ByteView{data.data(), data.size()}, offset, out);
}

bool ReadUint32(ByteView data, std::size_t& offset, std::uint32_t& out) {
  if (!data.data || offset + 4 > data.size) {
    return false;
  }
  out = static_cast<std::uint32_t>(data.data[offset]) |
        (static_cast<std::uint32_t>(data.data[offset + 1]) << 8) |
        (static_cast<std::uint32_t>(data.data[offset + 2]) << 16) |
        (static_cast<std::uint32_t>(data.data[offset + 3]) << 24);
  offset += 4;
  return true;
}
...
bool ReadUint64(const std::vector<std::uint8_t>& data, std::size_t& offset,
                std::uint64_t& out) {
  return ReadUint64(ByteView{data.data(), data.size()}, offset, out);
}

bool ReadUint64(ByteView data, std::size_t& offset, std::uint64_t& out) {
  if (!data.data || offset + 8 > data.size) {
    return false;
  }
  out = 0;
  for (int i = 0; i < 8; ++i) {
    out |= (static_cast<std::uint64_t>(data.data[offset + static_cast<std::size_t>(i)])
            << (i * 8));
  }
  offset += 8;
  return true;
