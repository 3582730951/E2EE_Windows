...
std::filesystem::path ResolveConfigDir(const std::string& config_path) {
  ...
  if (dir.is_relative()) {
    std::error_code ec;
    const auto cwd = pfs::CurrentPath(ec);
    if (!cwd.empty()) {
      dir = cwd / dir;
    }
  }
...
bool LoadClientConfig(const std::string& path, ClientConfig& out_cfg,
  ...
  while (std::getline(f, line)) {
    ...
    if (section == "client") {
      ...
      if (key == "audio_delay_ms") {
        ParseUint32(val, out_cfg.media.audio_delay_ms);
      } else if (key == "video_delay_ms") {
        ParseUint32(val, out_cfg.media.video_delay_ms);
      } else if (key == "audio_max_frames") {
        ParseUint32(val, out_cfg.media.audio_max_frames);
      } else if (key == "video_max_frames") {
        ParseUint32(val, out_cfg.media.video_max_frames);
      } else if (key == "pull_max_packets") {
        ParseUint32(val, out_cfg.media.pull_max_packets);
      } else if (key == "pull_wait_ms") {
        ParseUint32(val, out_cfg.media.pull_wait_ms);
      } else if (key == "group_pull_max_packets") {
        ParseUint32(val, out_cfg.media.group_pull_max_packets);
      } else if (key == "group_pull_wait_ms") {
        ParseUint32(val, out_cfg.media.group_pull_wait_ms);
      }
    }
  }
  ...
  if (out_cfg.media.video_delay_ms == 0) {
    out_cfg.media.video_delay_ms = 120;
  }
  if (out_cfg.media.audio_max_frames == 0) {
    out_cfg.media.audio_max_frames = 256;
  }
  if (out_cfg.media.video_max_frames == 0) {
    out_cfg.media.video_max_frames = 256;
  }
  if (out_cfg.media.pull_max_packets == 0) {
    out_cfg.media.pull_max_packets = 32;
  } else if (out_cfg.media.pull_max_packets > 256) {
    out_cfg.media.pull_max_packets = 256;
  }
  if (out_cfg.media.group_pull_max_packets == 0) {
    out_cfg.media.group_pull_max_packets = 64;
  } else if (out_cfg.media.group_pull_max_packets > 256) {
    out_cfg.media.group_pull_max_packets = 256;
  }
  if (out_cfg.media.pull_wait_ms > 1000) {
    out_cfg.media.pull_wait_ms = 1000;
  }
  if (out_cfg.media.group_pull_wait_ms > 1000) {
    out_cfg.media.group_pull_wait_ms = 1000;
  }
  return true;
}
