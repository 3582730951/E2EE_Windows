...
void CopyHistoryFilesIfMissing(const std::filesystem::path& from,
  ...
  std::error_code ec;
  if (!pfs::Exists(from, ec) || ec) {
    return;
  }
  pfs::CreateDirectories(to, ec);
  for (const auto& entry : std::filesystem::directory_iterator(from, ec)) {
    ...
    if (pfs::Exists(target, ec)) {
      ec.clear();
      continue;
    }
    pfs::CopyFile(entry.path(), target, false, ec);
    ec.clear();
  }
}
...
bool AcquireFileLock(const std::filesystem::path& path,
  ...
  const auto status = pfs::AcquireExclusiveFileLock(path, state->lock);
  if (status != pfs::FileLockStatus::kOk) {
    error = (status == pfs::FileLockStatus::kBusy) ? "history lock busy"
                                                   : "history lock failed";
    return false;
  }
  state->path = path;
...
void BestEffortWipeFile(const std::filesystem::path& path) {
  ...
  const std::uintmax_t size = pfs::FileSize(path, ec);
  if (!ec && size > 0) {
    std::fstream io(path, std::ios::binary | std::ios::in | std::ios::out);
    if (io) {
...
bool EncodeCompressionLayer(const std::vector<std::uint8_t>& plain,
  ...
  const std::size_t comp_capacity = static_cast<std::size_t>(bound);
  mi::common::ScopedBuffer comp_buf(mi::common::GlobalByteBufferPool(),
                                    comp_capacity, true);
  auto& comp = comp_buf.get();
  comp.resize(comp_capacity);
...
bool DecodeCompressionLayer(const std::vector<std::uint8_t>& input,
  ...
  const int rc = mz_uncompress(out_plain.data(), &dest_len, comp.data(),
...
bool DecodeInnerRecordPlain(const std::array<std::uint8_t, 32>& conv_key,
  ...
  auto& pool = mi::common::GlobalByteBufferPool();
  mi::common::ScopedBuffer plain_buf(pool, inner_cipher.size(), true);
  auto& plain = plain_buf.get();
  plain.resize(inner_cipher.size());
  ...
  mi::common::ScopedBuffer padded_buf(pool, plain.size(), true);
  ...
  (void)used_aes;
  mi::common::ScopedBuffer unpadded_buf(pool, padded.size(), true);
  auto& unpadded = unpadded_buf.get();
  std::string pad_err;
  ...
  std::string comp_err;
  if (!DecodeCompressionLayer(unpadded, out_plain, used_compress,
                              comp_err)) {
...
bool ParseFileSummaryRecord(const std::vector<std::uint8_t>& record_plain,
  ...
  if (version >= kMetaFileSummaryVersion && conv_count > 0) {
    ...
    if (out_conv_stats) {
      out_conv_stats->resize(conv_count);
      for (std::uint32_t i = 0; i < conv_count; ++i) {
        if (!mi::server::proto::ReadUint64(record_plain, off,
                                           (*out_conv_stats)[i].min_ts) ||
            !mi::server::proto::ReadUint64(record_plain, off,
                                           (*out_conv_stats)[i].max_ts) ||
            !mi::server::proto::ReadUint64(record_plain, off,
                                           (*out_conv_stats)[i].record_count) ||
            !mi::server::proto::ReadUint64(record_plain, off,
                                           (*out_conv_stats)[i].message_count)) {
          return false;
        }
      }
    } else {
      ChatHistoryConvStats scratch{};
      for (std::uint32_t i = 0; i < conv_count; ++i) {
        if (!mi::server::proto::ReadUint64(record_plain, off, scratch.min_ts) ||
            !mi::server::proto::ReadUint64(record_plain, off, scratch.max_ts) ||
            !mi::server::proto::ReadUint64(record_plain, off,
                                           scratch.record_count) ||
            !mi::server::proto::ReadUint64(record_plain, off,
                                           scratch.message_count)) {
          return false;
        }
      }
    }
...
bool DecryptOuterBlob(const std::array<std::uint8_t, 32>& master_key,
  ...
  mi::common::ScopedBuffer cipher_buf(mi::common::GlobalByteBufferPool(),
                                     cipher_len, false);
  ...
  mi::common::ScopedBuffer outer_plain_buf(mi::common::GlobalByteBufferPool(),
                                          cipher.size(), true);
  auto& outer_plain = outer_plain_buf.get();
  outer_plain.resize(cipher.size());
...
bool BuildLinearBijection(const std::array<std::uint8_t, 32>& key,
  ...
  std::array<std::uint8_t, 8> inv{};
  bool ok = false;
  const std::size_t label_len = (label && *label) ? std::strlen(label) : 0;
  std::vector<std::uint8_t> seed;
  seed.reserve(label_len + key.size() + 12);
  for (std::uint32_t attempt = 0; attempt < 1024; ++attempt) {
    seed.clear();
    if (label_len != 0) {
      seed.insert(seed.end(), label, label + label_len);
    }
...
bool DecodeAesLayer(const std::array<std::uint8_t, 32>& conv_key,
  ...
  if (!Aes256GcmDecrypt(aes_key, nonce, cipher, tag, out_plain, error)) {
    crypto_wipe(aes_key.data(), aes_key.size());
    return false;
  }
  crypto_wipe(aes_key.data(), aes_key.size());
...
bool WriteEncryptedRecord(std::ofstream& out,
  ...
  auto& pool = mi::common::GlobalByteBufferPool();
  mi::common::ScopedBuffer compressed_buf(pool, inner_plain.size(), true);
  auto& compressed = compressed_buf.get();
  if (!EncodeCompressionLayer(inner_plain, compressed, error)) {
    return false;
  }

  mi::common::ScopedBuffer padded_buf(
      pool, compressed.size() + kPadHeaderBytes, true);
  auto& padded = padded_buf.get();
  if (!PadPlain(compressed, padded, error)) {
    return false;
  }

  mi::common::ScopedBuffer aes_layer_buf(
      pool, padded.size() + kAesLayerHeaderBytes, false);
  auto& aes_layer = aes_layer_buf.get();
  ...
  mi::common::ScopedBuffer inner_cipher_buf(pool, aes_layer.size(), false);
  auto& inner_cipher = inner_cipher_buf.get();
  inner_cipher.resize(aes_layer.size());
  ...
  const std::size_t outer_plain_reserve =
      1 + 2 + conv_id.size() + inner_nonce.size() + 4 + inner_cipher.size() +
      inner_mac.size();
  mi::common::ScopedBuffer outer_plain_buf(pool, outer_plain_reserve, false);
  auto& outer_plain = outer_plain_buf.get();
  outer_plain.reserve(outer_plain_reserve);
  ...
  mi::common::ScopedBuffer outer_cipher_buf(pool, outer_plain.size(), false);
  auto& outer_cipher = outer_cipher_buf.get();
  outer_cipher.resize(outer_plain.size());
  ...
  const std::size_t outer_blob_reserve =
      outer_nonce.size() + outer_cipher.size() + outer_mac.size();
  mi::common::ScopedBuffer outer_blob_buf(pool, outer_blob_reserve, false);
  auto& outer_blob = outer_blob_buf.get();
  outer_blob.reserve(outer_blob_reserve);
...
bool ReadOuterRecordV2(std::ifstream& in,
  ...
  mi::common::ScopedBuffer record_buf(mi::common::GlobalByteBufferPool(),
                                     record_len, false);
  ...
  off += wrap_nonce.size();
  mi::common::ScopedBuffer wrap_cipher_buf(mi::common::GlobalByteBufferPool(),
                                          record_len, false);
  auto& wrap_cipher = wrap_cipher_buf.get();
  ...
  mi::common::ScopedBuffer outer_blob_buf(mi::common::GlobalByteBufferPool(),
                                         wrap_cipher.size(), true);
  auto& outer_blob = outer_blob_buf.get();
  outer_blob.resize(wrap_cipher.size());
...
bool ChatHistoryStore::Init(const std::filesystem::path& e2ee_state_dir,
  ...
  std::error_code ec;
  pfs::CreateDirectories(legacy_conv_dir_, ec);
  pfs::CreateDirectories(history_root, ec);
  pfs::CreateDirectories(history_dir_, ec);
  pfs::CreateDirectories(user_dir_, ec);
  pfs::CreateDirectories(attachments_dir_, ec);
  ...
  if (!pfs::Exists(key_path_, key_ec)) {
    const auto legacy_key = legacy_user_dir / "history_key.bin";
    if (pfs::Exists(legacy_key, key_ec) && !key_ec) {
      pfs::CreateDirectories(user_dir_, key_ec);
      pfs::CopyFile(legacy_key, key_path_, true, key_ec);
    }
  }
...
bool ChatHistoryStore::EnsureKeyLoaded(std::string& error) {
  ...
  std::error_code ec;
  bool key_file_exists = false;
  std::uint64_t key_file_size = 0;
  if (pfs::Exists(key_path_, ec)) {
    key_file_exists = true;
    if (ec) {
      error = "history key path error";
      return false;
    }
    key_file_size = pfs::FileSize(key_path_, ec);
    if (ec) {
      error = "history key size stat failed";
      return false;
    }
    if (key_file_size > kMaxHistoryKeyFileBytes) {
      error = "history key too large";
      return false;
    }
    std::string perm_err;
    if (!mi::shard::security::CheckPathNotWorldWritable(key_path_, perm_err)) {
      error = perm_err.empty() ? "history key permissions insecure" : perm_err;
      return false;
    }
  } else if (ec) {
    error = "history key path error";
  ...
  auto& pool = mi::common::GlobalByteBufferPool();
  mi::common::ScopedBuffer bytes_buf(
      pool, static_cast<std::size_t>(key_file_size), false);
  auto& bytes = bytes_buf.get();
  bytes.resize(static_cast<std::size_t>(key_file_size));
  if (key_file_exists) {
    std::ifstream f(key_path_, std::ios::binary);
    if (!f.is_open() || !ReadExact(f, bytes.data(), bytes.size())) {
      error = "history key read failed";
      return false;
    }
  }
  ...
  if (!bytes.empty()) {
    ...
    if (!was_dpapi) {
      ...
      if (!ProtectSecureStore(plain, kMagic, kEntropy, wrapped, wrap_err)) {
        error = wrap_err.empty() ? "history key rewrap failed" : wrap_err;
        return false;
      }
      std::error_code write_ec;
      if (!pfs::AtomicWrite(key_path_, wrapped.data(), wrapped.size(),
                            write_ec)) {
        error = "history key rewrap failed";
        return false;
      }
#ifndef _WIN32
      {
  ...
  std::error_code ec2;
  pfs::CreateDirectories(user_dir_, ec2);
  std::string perm_err;
  if (!mi::shard::security::CheckPathNotWorldWritable(key_path_, perm_err)) {
    error = perm_err.empty() ? "history key permissions insecure" : perm_err;
    return false;
  ...
  std::error_code write_ec;
  if (!pfs::AtomicWrite(key_path_, out_bytes.data(), out_bytes.size(),
                        write_ec)) {
...
bool ChatHistoryStore::EnsureTagKeyLoaded(std::string& error) {
  ...
  bool tag_file_exists = false;
  std::uint64_t tag_file_size = 0;
  if (pfs::Exists(tag_key_path_, ec)) {
    tag_file_exists = true;
    if (ec) {
      error = "history tag key path error";
      return false;
    }
    tag_file_size = pfs::FileSize(tag_key_path_, ec);
    if (ec) {
      error = "history tag key size stat failed";
      return false;
    }
    if (tag_file_size > kMaxHistoryKeyFileBytes) {
      error = "history tag key too large";
      return false;
    }
    std::string perm_err;
    if (!mi::shard::security::CheckPathNotWorldWritable(tag_key_path_,
                                                        perm_err)) {
      error =
          perm_err.empty() ? "history tag key permissions insecure" : perm_err;
      return false;
    }
  } else if (ec) {
    error = "history tag key path error";
  ...
  if (tag_file_size >
      static_cast<std::uint64_t>((std::numeric_limits<std::size_t>::max)())) {
    error = "history tag key size invalid";
    return false;
  }

  auto& pool = mi::common::GlobalByteBufferPool();
  mi::common::ScopedBuffer bytes_buf(
      pool, static_cast<std::size_t>(tag_file_size), false);
  auto& bytes = bytes_buf.get();
  bytes.resize(static_cast<std::size_t>(tag_file_size));
  if (tag_file_exists) {
    std::ifstream f(tag_key_path_, std::ios::binary);
    if (!f.is_open() || !ReadExact(f, bytes.data(), bytes.size())) {
      error = "history tag key read failed";
      return false;
    }
  }
  ...
  const bool need_write = !was_dpapi || bytes.empty();
  if (need_write) {
    std::string wrap_err;
    if (!ProtectSecureStore(plain, kMagic, kEntropy, out_bytes, wrap_err)) {
      error = wrap_err.empty() ? "history tag key protect failed" : wrap_err;
      return false;
    }
  } else {
    out_bytes = bytes;
  ...
  if (need_write) {
    ...
    std::error_code write_ec;
    if (!pfs::AtomicWrite(tag_key_path_, out_bytes.data(), out_bytes.size(),
                          write_ec)) {
...
bool ChatHistoryStore::EnsureProfileLoaded(const std::string& username,
  ...
  std::error_code ec;
  if (pfs::Exists(profiles_path_, ec)) {
    has_file = !ec;
  }
  if (has_file) {
    const std::uint64_t size64 = pfs::FileSize(profiles_path_, ec);
    if (ec || size64 == 0 ||
    ...
    auto& pool = mi::common::GlobalByteBufferPool();
    mi::common::ScopedBuffer bytes_buf(pool, size, false);
    auto& bytes = bytes_buf.get();
    bytes.resize(size);
    {
      std::ifstream in(profiles_path_, std::ios::binary);
      if (!in.is_open() || !ReadExact(in, bytes.data(), bytes.size())) {
        error = "history profiles read failed";
        ReleaseFileLock(lock);
        return false;
      }
  ...
  if (updated || !has_file) {
    ...
    pfs::Rename(tmp, profiles_path_, ec);
    if (ec) {
      pfs::Remove(tmp, ec);
      error = "history profiles write failed";
      ReleaseFileLock(lock);
...
bool ChatHistoryStore::LoadHistoryIndex(std::string& error) {
  ...
  const std::uint64_t size64 = pfs::FileSize(index_path_, ec);
  if (ec || size64 == 0 ||
      size64 >
          static_cast<std::uint64_t>((std::numeric_limits<std::size_t>::max)())) {
    return false;
  ...
  bytes.resize(size);
  {
    ...
    if (!ReadExact(in, bytes.data(), bytes.size())) {
      return false;
    }
  }
  ...
  for (std::uint32_t i = 0; i < file_count; ++i) {
    ...
    if (!pfs::Exists(entry.path, ec) || ec) {
      return false;
    }
...
bool ChatHistoryStore::SaveHistoryIndex(std::string& error) {
  ...
  for (auto& entry : history_files_) {
    ...
    conv_hashes.assign(entry.conv_hashes.begin(), entry.conv_hashes.end());
    ...
    conv_keys.insert(conv_keys.end(), entry.conv_keys.begin(),
                     entry.conv_keys.end());
...
bool ChatHistoryStore::LoadAttachmentsIndex(std::string& error) {
  ...
  std::error_code ec;
  if (!pfs::Exists(attachments_index_path_, ec)) {
    return true;
  }
  if (ec) {
  ...
  const std::uint64_t size64 = pfs::FileSize(attachments_index_path_, ec);
  if (ec || size64 == 0 ||
      size64 >
          static_cast<std::uint64_t>((std::numeric_limits<std::size_t>::max)())) {
    error = "attachments index read failed";
...
bool ChatHistoryStore::SaveAttachmentsIndex(std::string& error) {
  ...
  pfs::Rename(tmp, attachments_index_path_, ec);
  if (ec) {
    pfs::Remove(tmp, ec);
    error = "attachments index write failed";
    return false;
...
bool ChatHistoryStore::UpdateAttachmentPreview(
  ...
  pfs::Rename(tmp, preview_path, ec);
  if (ec) {
    pfs::Remove(tmp, ec);
    error = "attachments preview write failed";
    return false;
...
bool ChatHistoryStore::AppendHistoryJournal(const std::vector<std::uint8_t>& plain,
  ...
  auto& pool = mi::common::GlobalByteBufferPool();
  mi::common::ScopedBuffer cipher_buf(pool, plain.size(), false);
  auto& cipher = cipher_buf.get();
...
bool ChatHistoryStore::LoadHistoryJournal(std::string& error) {
  ...
  std::error_code ec;
  if (!pfs::Exists(journal_path_, ec) || ec) {
    return false;
  }
...
bool ChatHistoryStore::MigrateLegacyHistoryFiles(const std::string& legacy_tag,
  ...
  for (const auto& entry :
    ...
    pfs::Rename(entry.path(), target, ec);
    if (ec) {
      ok = false;
      ec.clear();
    }
  ...
  if (!candidates.empty()) {
    ...
    auto worker = [&]() {
      for (;;) {
        ...
        if (conv_hint > 0) {
          conv_hashes.reserve(conv_hint);
          conv_stats.reserve(conv_hint);
        }
        for (int pass = 0; pass < 8; ++pass) {
          ...
          if (ParseFileSummaryRecord(record_plain, meta_seq, meta_uuid,
            ...
            file.conv_hashes.assign(conv_hashes.begin(), conv_hashes.end());
...
bool ChatHistoryStore::EnsureHistoryFile(
  ...
  if (it != conv_to_file_.end() && it->second < history_files_.size()) {
    ...
    if (!new_name.empty()) {
      ...
      if (!pfs::Exists(new_path, ec)) {
        pfs::Rename(entry.path, new_path, ec);
        if (!ec) {
          entry.path = new_path;
          entry.tag = user_tag_;
  ...
  const auto append_messages =
      [&](const std::vector<ChatHistoryMessage>& messages) {
        ...
        auto& pool = mi::common::GlobalByteBufferPool();
        mi::common::ScopedBuffer rec_buf(pool, 0, true);
        auto& rec = rec_buf.get();
...
bool ChatHistoryStore::LoadLegacyConversation(
  ...
  auto& pool = mi::common::GlobalByteBufferPool();
  mi::common::ScopedBuffer plain_buf(pool, 0, true);
  auto& plain = plain_buf.get();
  while (true) {
    plain.clear();
    std::string rec_err;
    ...
    if (kind == kMessageKindEnvelope) {
      ...
      if (off < plain.size()) {
        ...
        if (mi::server::proto::ReadBytes(plain, summary_off, m.summary) &&
    ...
    if (kind == kMessageKindSystem) {
      if (!mi::server::proto::ReadString(plain, off, m.system_text_utf8) ||
          off != plain.size()) {
        continue;
      }
      m.is_system = true;
...
bool ChatHistoryStore::AppendEnvelope(bool is_group,
  ...
  auto& pool = mi::common::GlobalByteBufferPool();
  mi::common::ScopedBuffer summary_buf(pool, 0, true);
  mi::common::ScopedBuffer rec_buf(pool, 0, true);
  auto& summary = summary_buf.get();
  auto& rec = rec_buf.get();
  summary.clear();
  rec.clear();
...
bool ChatHistoryStore::DeleteConversation(bool is_group,
  ...
  auto rewrite_file = [&](HistoryFileEntry& entry) -> bool {
    ...
    std::error_code ec;
    pfs::Remove(bak_path, ec);
    pfs::Rename(entry.path, bak_path, ec);
    if (ec) {
      pfs::Remove(tmp_path, ec);
      error = "history write failed";
      return false;
    }
    pfs::Rename(tmp_path, entry.path, ec);
    if (ec) {
      pfs::Rename(bak_path, entry.path, ec);
      pfs::Remove(tmp_path, ec);
      error = "history write failed";
      return false;
    }
...
bool ChatHistoryStore::ClearAll(bool delete_attachments,
  ...
  std::error_code ec;
  if (pfs::Exists(history_dir_, ec) && !ec) {
    for (const auto& entry : std::filesystem::directory_iterator(history_dir_, ec)) {
      if (ec) {
        break;
      }
  ...
  if (delete_attachments) {
    if (!attachments_index_path_.empty()) {
      if (secure_wipe) {
        ...
        pfs::Remove(attachments_index_path_, ec);
      }
    }
    if (!attachments_dir_.empty() && pfs::Exists(attachments_dir_, ec) &&
        !ec) {
      for (const auto& entry :
      ...
      pfs::RemoveAll(attachments_dir_, ec);
    }
  }
...
bool ChatHistoryStore::LoadConversation(bool is_group,
  ...
  while (true) {
    ...
    if (kind == kMessageKindEnvelope) {
      ...
      if (off < record_plain.size()) {
        std::size_t summary_off = off;
        if (mi::server::proto::ReadBytes(record_plain, summary_off, m.summary) &&
    ...
    if (kind == kMessageKindSystem) {
      if (!mi::server::proto::ReadString(record_plain, off,
                                         m.system_text_utf8) ||
