...
using padding::PadPayload;
using padding::UnpadPayload;
...
std::string FingerprintPeer(const std::uint8_t* id_sig_pk,
  ...
  return mi::common::Sha256Hex(buf.data(), buf.size());
}
...
bool ReadAll(const std::filesystem::path& path, std::vector<std::uint8_t>& out,
  ...
  if (!mi::shard::security::CheckPathNotWorldWritable(path, perm_err)) {
    error = perm_err.empty() ? "identity permissions insecure" : perm_err;
    return false;
  }
...
bool WriteAll(const std::filesystem::path& path,
  ...
  if (!pfs::AtomicWrite(path, data.data(), data.size(), write_ec)) {
    error = "write failed";
    return false;
  }
#ifndef _WIN32
...
std::string BuildIdentityEntropyV2() {
  ...
  const std::string machine_id = mi::platform::MachineId();
  if (!machine_id.empty()) {
    entropy.push_back(':');
...
bool UnwrapIdentityDpapi(const std::vector<std::uint8_t>& in,
  ...
  if (!MaybeUnprotectSecureStore(in, magic, entropy.c_str(), out_plain, was_dpapi,
                           error)) {
    return false;
  }
  if (!was_dpapi) {
...
bool WrapIdentityDpapi(const std::vector<std::uint8_t>& plain,
                       ...
                       std::string& error) {
  return ProtectSecureStore(plain, magic, entropy.c_str(), out_wrapped, error);
}
...
bool WrapIdentityTpm(const std::vector<std::uint8_t>& plain,
  ...
  [[maybe_unused]] mi::common::ScopedWipe wipe_data_key(data_key);
  ...
  if (!mi::platform::TpmWrapKey(data_key, wrapped_key, error)) {
...
bool UnwrapIdentityTpm(const std::vector<std::uint8_t>& in,
  ...
  std::array<std::uint8_t, 32> data_key{};
  [[maybe_unused]] mi::common::ScopedWipe wipe_data_key(data_key);
  if (!mi::platform::TpmUnwrapKey(wrapped_key, data_key, error)) {
    return false;
  }
...
bool Engine::Init(const std::filesystem::path& state_dir, std::string& error) {
  ...
  std::error_code ec;
  pfs::CreateDirectories(state_dir_, ec);
...
bool Engine::LoadOrCreateIdentity(std::string& error) {
  ...
  if (exists) {
    ...
    if (identity_policy_.tpm_enable && mi::platform::TpmSupported() &&
...
bool Engine::TrustPendingPeer(const std::string& pin, std::string& error) {
  ...
  const std::string expected_pin = security::NormalizeCode(pending_.pin6);
...
bool Engine::CheckTrustedForSend(const std::string& peer_username,
  ...
  if (!mi::common::ConstantTimeEqual(it->second, fingerprint_hex)) {
    SetPendingTrust(peer_username, fingerprint_hex);
...
bool Engine::DecryptFromPayload(const std::string& peer_username,
  ...
  if (msg_type == kMsgPreKey) {
    ...
    if (trust_it == trusted_peers_.end() ||
        !mi::common::ConstantTimeEqual(trust_it->second, fingerprint)) {
