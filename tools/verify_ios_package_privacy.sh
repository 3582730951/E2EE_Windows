...
scan_names() {
  ...
  hit="$(
    find "$root" -print 2>/dev/null | while IFS= read -r path; do
      ...
      case "$base" in
        *.log|*.log.*|*.dmp|*.dump|*.crash|*.ips|*.dsym|crash|crashes|telemetry|diagnostic|diagnostics|metrics|metric|audit|audits|ops_health|*crashlytics*|*firebase*|*firebase-analytics*|*telemetry*|*diagnostic*|*diagnostics*|*metrics*|*ops_health*|*audit*|*sentry*|*bugsnag*|*datadog*|*newrelic*|*appcenter*)
          printf '%s\n' "$path"
          break
          ;;
      esac
