...
if "%ops_choice%"=="2" (
  powershell -NoProfile -ExecutionPolicy Bypass -Command "$p='%CONFIG_PATH%'; $en='ops_'+'enable'; $remote='ops_'+'allow_remote'; $tk='ops_'+'token'; $secret=[Guid]::NewGuid().ToString('N')+[Guid]::NewGuid().ToString('N'); $text=Get-Content $p -Raw; $text=$text -replace ('(?m)^'+$en+'.*$'), ($en+'=1'); $text=$text -replace ('(?m)^'+$remote+'.*$'), ($remote+'=0'); if($text -match ('(?m)^'+$tk+'=')){ $text=$text -replace ('(?m)^'+$tk+'.*$'), ($tk+'='+$secret) } else { $text=$text -replace '(?m)^\[kcp\]', ($tk+'='+$secret+\"`n[kcp]\") }; Set-Content -Encoding utf8 $p $text"
) else if "%ops_choice%"=="3" (
  powershell -NoProfile -ExecutionPolicy Bypass -Command "$p='%CONFIG_PATH%'; $en='ops_'+'enable'; $remote='ops_'+'allow_remote'; $tk='ops_'+'token'; $secret=[Guid]::NewGuid().ToString('N')+[Guid]::NewGuid().ToString('N'); $text=Get-Content $p -Raw; $text=$text -replace ('(?m)^'+$en+'.*$'), ($en+'=1'); $text=$text -replace ('(?m)^'+$remote+'.*$'), ($remote+'=1'); if($text -match ('(?m)^'+$tk+'=')){ $text=$text -replace ('(?m)^'+$tk+'.*$'), ($tk+'='+$secret) } else { $text=$text -replace '(?m)^\[kcp\]', ($tk+'='+$secret+\"`n[kcp]\") }; Set-Content -Encoding utf8 $p $text"
) else (
