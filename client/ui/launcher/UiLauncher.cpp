...
int WINAPI wWinMain(HINSTANCE, HINSTANCE, PWSTR cmdLine, int) {
    ...
    const std::wstring runtimeDir = rootDir + L"\\runtime";
    const std::wstring appExe = runtimeDir + L"\\mi_e2ee_app.exe";
    ...
    if (DirExists(runtimeDir)) {
        ...
        const std::wstring pluginDir = runtimeDir + L"\\plugins";
        const std::wstring platformDir = pluginDir + L"\\platforms";
        if (DirExists(pluginDir)) {
            SetEnvIfEmpty(L"QT_PLUGIN_PATH", pluginDir);
        }
