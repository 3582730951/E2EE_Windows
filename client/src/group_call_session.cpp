...
GroupCallSession::GroupCallSession(mi::client::ClientCore& core,
...
bool GroupCallSession::SetActiveKey(std::uint32_t key_id, std::string& error) {
  ...
  if (!core_.GetGroupCallKey(config_.group_id, config_.call_id, key_id,
                             call_key)) {
...
bool GroupCallSession::SendFrame(mi::media::StreamKind kind,
  ...
  if (!core_.PushGroupMedia(config_.group_id, config_.call_id, *packet)) {
    return false;
  }
  return true;
}
...
bool GroupCallSession::PollIncoming(std::uint32_t max_packets,
  ...
  const auto packets = core_.PullGroupMedia(config_.call_id, max_packets, wait_ms);
  if (packets.empty() && !core_.last_error().empty()) {
    error = core_.last_error();
    return false;
  }
