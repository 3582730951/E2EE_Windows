...
MediaSession::MediaSession(mi::client::ClientCore& core,
...
bool MediaSession::Init(std::string& error) {
  ...
  if (!core_.DeriveMediaRoot(config_.peer_username, config_.call_id, media_root_,
                             error)) {
    if (error.empty()) {
...
bool MediaSession::PollIncoming(std::uint32_t max_packets,
  ...
  const auto packets = core_.PullMedia(config_.call_id, max_packets, wait_ms);
